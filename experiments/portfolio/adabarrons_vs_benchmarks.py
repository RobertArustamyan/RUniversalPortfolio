...
if __name__ == '__main__':
    ...
    fig_name = 'ada_barrons_4stocks'

    continue_wealth_from_training = False

    data_dict = prepare_stock_data(
        stocks=["AAPL", "TSLA", "MSFT", "NVDA", "GOOGL", "AMZN", "NFLX"],
        train_days=2000,
        test_days=700
    )
    ...
    print(f"Training completed with {train_results['restart_count']} restarts")
    print(f"Final beta after training: {train_results['final_beta']:.6f}")
    ...
    for day, price_rel in enumerate(test_price_relatives):
        ...
        # Update portfolio with the portfolio that was used
        portfolio.update(price_rel, current_portfolio)

        if (day + 1) % 10 == 0 or day == 0:
            print(f"Test Day {day + 1}: Wealth = {test_wealth_current:.4f}, Beta = {portfolio.beta:.6f}, " +
                  f"Portfolio = [{', '.join([f'{x:.3f}' for x in current_portfolio])}]")

    print(f"\nTesting completed with {portfolio.restart_count - train_results['restart_count']} additional restarts")
    print(f"Total restarts: {portfolio.restart_count}")
    print(f"Final beta: {portfolio.beta:.6f}")
    ...
    if continue_wealth_from_training:
        ...
        ada_barrons_results = {
            'strategy': 'Ada-Barrons (trained on 2yr)',
    ...
    results = compare_strategies(
        ...
        additional_results=ada_barrons_results
    )